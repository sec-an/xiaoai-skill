...
class XiaoAIUser(XiaoAIBase):
    _fields = [('user_id', str, True), ('access_token', str, False), ('is_user_login', bool, False),
               ('gender', str, False), ('real_user_id', str, False)]


class XiaoAISession(XiaoAIBase):
    _fields = [('is_new', bool, True), ('session_id', str, True), ('application', XiaoAIApplication, True),
               ('attributes', dict, False), ('user', XiaoAIUser, False)]
...
class XiaoAIContext(XiaoAIBase):
    _fields = [('passport', dict, False), ('device_id', str, False), ('bind_id', str, False), ('app_info', list, False),
...
class XiaoAIRequest(XiaoAIBase):
    _fields = [('request_id', str, True), ('timestamp', int, True), ('locale', str, True),
               ('intent', XiaoAIIntent, False), ('no_response', bool, False), ('type', int, True),
               ('slot_info', XiaoAISlotInfo, False), ('event_type', str, False),
               ('event_property', XiaoAIEventProperty, False)]


class XiaoAIOpenRequest(XiaoAIBase):
    _fields = [('version', str, True), ('query', str, False), ('session', XiaoAISession, True),
               ('context', XiaoAIContext, False), ('request', XiaoAIRequest, True)]
...
class XiaoAIActionProperty:
    def __init__(self, file_id_list=None, quick_app_path=None, push_app_info=None, quick_app_extra=None,
                 device_command=None, app_intent_info=None, quick_app_card_info=None, app_h5_url=None):
...
def xiaoai_response(data):
    return json.dumps(data, default=lambda obj: dict(
        ("type" if key == "type_" else key, value) for (key, value) in obj.__dict__.items() if value is not None))
